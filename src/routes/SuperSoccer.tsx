...
export default function SuperSoccer() {
  useEffect(() => {
    ...
    function populateObj(q1, q2, q3, q4) {
      if (questionList[0]) {
        let answerList = shuffleArrary(Object.keys(question[questionList[0]]));
    ...
    function movingLines() {
      ...
      if (!stopLine) {
        ...
        if (distance > semicircleRadius) {
          let angle = Math.atan2(dy, dx);
          pausedLineX = semicircleCenterX + semicircleRadius * Math.cos(angle);
          pausedLineY = semicircleCenterY + semicircleRadius * Math.sin(angle);
        }
  ...
  return (
    <div className="soccer-bg">
      <h1 className="soccer-text">Welcome to SOCCOR</h1>
      <h2 className="soccer-text">
        hit the "space bar" when the arrow land on the right answer
      </h2>
